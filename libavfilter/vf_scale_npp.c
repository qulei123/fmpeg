...
static int nppscale_fmtconv(AVFilterContext *ctx, NPPScaleStageContext *stage,
                                AVFrame *out, AVFrame *in)
{
   AVHWFramesContext *out_frames_ctx = (AVHWFramesContext*)out->hw_frames_ctx->data;
   AVHWFramesContext *in_frames_ctx = (AVHWFramesContext*)in->hw_frames_ctx->data;
   ...
   case AV_PIX_FMT_BGR24:
        ...
        err = nppiNV12ToBGR_8u_P2C3R(pSrc,
                                     in->linesize[0],
                                     out->data[0],
                                     out->linesize[0],
                                    (NppiSize){ in->width, in->height });
   ...
   case AV_PIX_FMT_YUV420P:
       ...
       err = nppiNV12ToYUV420_8u_P2P3R(pSrc,
                                       in->linesize[0],
                                       out->data, out->linesize,
                                      (NppiSize){ in->width, in->height });
       break;
   ...
   if (err != NPP_SUCCESS) {
       av_log(ctx, AV_LOG_ERROR, "NPP color conversion error: %d\n", err);
       return AVERROR_UNKNOWN;
   }
